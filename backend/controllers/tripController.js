...
export const getTripPlan = async (req, res) => {
    try {
        const { destination, days } = req.body; // Extract destination and days from request body

        if (!destination || !days) {
            return res.status(400).json({ error: 'Please enter both destination and days' });
        }
        ...
        const prompt = `Plan a ${days}-day trip to ${destination}. Provide the daily itinerary in the following format:\n\n` +
               `Day 1:\n` +
               `Morning:\n` +
        ...
        // Split the response text by sections and store in an array
        const plan = responseText.split('\n').filter(line => line.trim() !== '');
         // Send response
         res.json({ tripPlan: plan });
    } catch (error) {
